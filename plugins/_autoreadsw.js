...
export async function before(m) {
   if (m.key.remoteJid === 'status@broadcast') {
    ...
    if (/protocolMessage/i.test(puks)) this.sendMessage(owner[0][0]+'@s.whatsapp.net', {text:'Status dari @'+m.key.participant.split('@')[0]+' Telah dihapus', mentions: [m.key.participant]}, {quoted: gfkontak})
